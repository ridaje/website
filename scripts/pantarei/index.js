...
/*
 * ISC License
 * Copyright (c) 2018 Enrico Marino
 */
...
class Director {
  ...
  parse (node) {
    ...
    this._parse_directed_nodes(node, node)
  }

  _parse_directed_nodes (parent, node) {
    for (let child of node.children) {
      this._parse_directed_node(parent, child)
      this._parse_directed_nodes(parent, child)
    }
  }
  ...
  _parse_directed_node (node, directed_node) {
    ...
    if (directed_node._directives.length > 0) {
      if (!node._directed_nodes.includes(directed_node)) {
        ...
        directed_node._director_node = node
      }
    }
  }
    ...
    while (bubble) {
      ...
      if (listeners) {
        ...
        if (listener) {
          ...
          let new_event = Object.create(event, {
            target: {
              value: target
            }
          })
          listener.call(node, new_event, event.detail)
        }
      }
...
class DirectiveRepeat extends Directive {
  ...
  static parse (node, attribute) {
    ...
    let content = node.content.firstElementChild
...
class DirectiveHtml extends Directive {
